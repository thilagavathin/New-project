...
if($group_id != 0){
    $sql = mysql_query("SELECT * FROM chats WHERE status='Y' AND group_id=".$group_id." ORDER BY created_at ASC") or die("Query Error");
...
if(isset($_POST['see']))
{
	$RS = "SELECT * FROM chats WHERE    (sender_user_id=".$sender_id." AND receiver_user_id=".$receiver_id.") OR (sender_user_id=".$receiver_id." AND receiver_user_id=".$sender_id.") AND group_id=".$group_id." ORDER BY created_at ASC";
}
else
{
$RS = "SELECT * FROM chats WHERE status='Y' AND  (sender_user_id=".$sender_id." AND receiver_user_id=".$receiver_id." AND send_status='1') OR (sender_user_id=".$receiver_id." AND receiver_user_id=".$sender_id." AND rec_status='1') AND group_id=".$group_id." ORDER BY created_at ASC";
   }
    
	$sql = mysql_query($RS); // or die("Query Error");
    $count = mysql_num_rows($sql);
    $html = "<ul>";
	   ...
	   foreach($rowss as $row)
		{
			if($row['sender_user_id']==$sender_id)
			{
				$sql = mysql_query("update chats set send_status='0' WHERE sender_user_id=".$sender_id." and receiver_user_id='$receiver_id' and send_status='1' AND group_id=".$group_id) or die("Query Error");
			}
			
			if($row['receiver_user_id']==$sender_id)
			{
				$sql = mysql_query("update chats set rec_status='0' WHERE receiver_user_id=".$sender_id." and sender_user_id='$receiver_id' and rec_status='1' AND group_id=".$group_id) or die("Query Error");
			}
