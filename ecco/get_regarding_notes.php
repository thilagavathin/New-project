...
$regarding_sql=mysql_query("SELECT comments,created_date, user_id FROM tta_regarding_notes WHERE contract_num='".$contract."' order by tta_regarding_notes.id DESC ");
...
$sql_reg=mysql_query("SELECT id FROM tta_regarding_status WHERE agency_id='".$agency."' AND user_id='".$user_id."' AND contract_num='".$contract."'");
$row_reg=mysql_fetch_row($sql_reg);

if($row_reg[0]) mysql_query("UPDATE tta_regarding_status SET comment_count=".$num_rows." WHERE agency_id=".$agency." AND user_id='".$user_id."' AND contract_num='".$contract."'");
else mysql_query("INSERT INTO tta_regarding_status (agency_id,user_id,contract_num,comment_count,updated_date) VALUES (".$agency.",".$user_id.",'".$contract."',".$num_rows.",'".$current_date."')");
?>
                                  </ul>
                                </div>
